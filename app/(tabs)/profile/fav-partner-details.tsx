...
const FavourtiePartnerDetails = () => {
  ...
  // ── Heart icon state — independent, always default true ─────────────────
  const [isLoved, setIsLoved] = useState(true);

  // ── Save/Unsave button state — controlled by API ─────────────────────────
  ...
  // ── Heart icon — only toggles isLoved, no API, no relation to button ─────
  const handleToggleFavorite = () => {
    setIsLoved((prev) => !prev);
  };

  // ── Save/Unsave button — API call, on success update isSaved ─────────────
  const handleSavePartner = async () => {
    ...
    try {
      ...
      router.dismissAll();
      router.replace("/(tabs)/profile/favorite-partners");
    } catch (error: any) {
        ...
        >
          ...
          >
            <View className="flex-row items-center justify-between">
              ...
              {/* ── Heart icon — isLoved state only, no relation to button ── */}
          ...
          >
            ...
            {/* Save / Unsave Partner — API call only */}
            <Pressable
...
export default FavourtiePartnerDetails;
